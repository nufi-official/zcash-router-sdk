...
export function SwapTimeline({
  ...
  // Build timeline steps - always show full timeline
  const buildTimeline = () => {
    return [
      'GETTING QUOTE',
      'DEPOSIT SENT',
  ...
  const getActiveStep = () => {
    ...
    if (!currentState) {
      return 1; // Quote received, move to step 1 (Deposit Sent)
    }
    ...
    // Return the step index, or 1 if not found
    return stepIndex !== undefined ? stepIndex : 1;
  };
